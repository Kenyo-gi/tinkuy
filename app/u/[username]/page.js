...
export default async function PublicCardPage({ params }) {
  ...
  const whatsappLink = profile.whatsapp_phone
    ? `https://wa.me/${profile.whatsapp_phone.replace(/\D/g, "")}`
    : null;
  ...
  return (
    <main className="min-h-screen bg-gray-50 px-4 py-10 flex justify-center">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow p-6 text-center">
        ...
        {profile.position_company && (
          <p className="text-gray-500 text-sm">{profile.position_company}</p>
        )}

        <div className="mt-6 space-y-2">
          
            href={`/api/vcard/${profile.username}`}
            className="block w-full bg-black text-white rounded-lg py-2.5 font-medium hover:bg-gray-800 transition"
          >
            Guardar contacto
          </a>
          ...
          {whatsappLink && (
              ...
              target="_blank"
              rel="noreferrer"
              className="block w-full bg-green-500 text-white rounded-lg py-2.5 font-medium hover:bg-green-600 transition"
            >
              WhatsApp
            </a>
          )}
          ...
          {profile.phone && (
              ...
              className="block w-full border border-gray-300 rounded-lg py-2.5 font-medium hover:bg-gray-50 transition"
            >
        ...
        <div className="mt-6 flex justify-center gap-2 flex-wrap">
          {SOCIALS.map(({ key, label, color }) =>
            profile[key] ? (
                ...
                target="_blank"
                rel="noreferrer"
                className={`${color} text-white text-xs font-medium rounded-full px-4 py-2 hover:opacity-90 transition`}
              >
                {label}
              </a>
            ) : null
